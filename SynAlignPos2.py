...
class SynAlign(Model):
    ...
    def get_batch(self, path, batch_size):
        ...
        source_sent, target_sent, source_pos_ids, target_pos_ids, source_mask, target_mask = \
            tf.py_func(self.batch_process, [batch, self.p.max_sent_len], [tf.int32, tf.int32, tf.float32, tf.float32, tf.bool, tf.bool])
    ...
    def add_model(self, source_sent, target_sent, source_pos_ids, target_pos_ids, source_mask, target_mask):
        ...
        source_sent_embed = tf.Print(source_sent_embed, [source_sent_embed], summarize=1000)


        # add position embedding
        source_pos_embedding = tf.matmul(source_pos_ids, tf.tile(tf.expand_dims(self.source_pos_emb_W, 0),
                                                                 [tf.shape(source_pos_ids)[0], 1, 1]))
        source_pos_embedding = tf.Print(source_pos_embedding, [source_pos_embedding], summarize=1000)
        target_pos_embedding = tf.matmul(target_pos_ids, tf.tile(tf.expand_dims(self.target_pos_emb_W, 0),
                                                                 [tf.shape(target_pos_ids)[0], 1, 1]))
        target_pos_embedding = tf.Print(target_pos_embedding, [target_pos_embedding], summarize=1000)

        # source_sent_embed = source_sent_embed + source_pos_embedding
        # target_sent_embed = target_sent_embed + target_pos_embedding
    ...
    def build_eval_graph(self):
        # get batch data
        eval_source_sent, eval_target_sent, eval_source_pos_ids, eval_target_pos_ids, eval_source_mask, eval_target_mask, self.train_iter =\
        ...
        # add position embedding
        source_pos_embedding = tf.matmul(eval_source_pos_ids, tf.tile(tf.expand_dims(self.source_pos_emb_W, 0),
                                                                      [tf.shape(eval_source_pos_ids)[0], 1, 1]))
        target_pos_embedding = tf.matmul(eval_target_pos_ids, tf.tile(tf.expand_dims(self.target_pos_emb_W, 0),
                                                                      [tf.shape(eval_target_pos_ids)[0], 1, 1]))
        source_sent_embed = source_sent_embed + source_pos_embedding
        target_sent_embed = target_sent_embed + target_pos_embedding
