...
class SynAlign(Model):
    ...
    def build_train_graph(self):
        ...
        s_mask_shift_list = []
        for i in range(1, self.p.num_neg + 1):
            s_sent_embed_shift_list.append(tf.roll(source_sent_embed, shift=[0, i, 0], axis=[0, 1, 2]))
            s_mask_shift_list.append(tf.roll(source_mask, shift=[0, i], axis=[0, 1]))
        ...
        t_mask_shift_list = []
        for i in range(1, self.p.num_neg + 1):
            t_sent_embed_shift_list.append(tf.roll(target_sent_embed, shift=[0, i, 0], axis=[0, 1, 2]))
            t_mask_shift_list.append(tf.roll(target_mask, shift=[0, i], axis=[0, 1]))
